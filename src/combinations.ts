...
export function combinations(): Powers[] {
  interface Abilities {main: number; sub: number}
  ...
  for (const p of primaryAbilities) {
    for (const s of secondaryAbilities) {
      const main: Ability = {primaryAbilities: p.main, secondaryAbilities: s.main};
      const sub: Ability = {primaryAbilities: p.sub, secondaryAbilities: s.sub};
